...
## What for?
Phonological description of the language is not formalized. There are a lot of events that are omitted during the standard phonological description:
...
Second, having such a list it is possible automatically create phonological descriptions of language using transcription to IPA correspondences Without such an algorithm, researchers need to do it by themselves, so it is more likely that mistakes will appear. Having corpora or word list it is possible automate the phonological description of language under investigation.

So `lingphonology` is planed to be an R package that automatically creates phonological description using text corpora or word list and correspondences for transcription used in the source to IPA symbols. It is also possible to start a work, investigating representativity of different word lists and corpora, that later will provide us a measure of our belief in phonological description.
...
* syllable structure strategies (with different generalizations based on phonological features) and its frequencies
